package com.aws.lambda.taskupdaterhook.scrum.tool.rally;
...
import com.aws.lambda.taskupdaterhook.scrum.response.UsersResponse;
import com.aws.lambda.taskupdaterhook.scrum.tool.rally.pojo.RallyTask;
import com.aws.lambda.taskupdaterhook.scrum.tool.rally.pojo.RallyUser;
...
public class Rally implements IScrumTool {
	...
	public TaskUpdateResponse updateTask(TaskUpdateRequest taskUpdateReq) throws ScrumException {
		...
		try {
			...
			gson = new Gson();
			Logger.log("Update rally response on trying to update task arg obj json= " + gson.toJson(response));
			...
			Logger.log("Exception exception occured in updateTask of Class Rally" + exp);
			System.out.println("Exception exception occured in updateTask of Class Rally" + exp);
			throw new ScrumException("Exception exception occured in updateTask of Class Rally", exp);
		} finally {
			if (null != restApi) {
				try {
					restApi.close();
